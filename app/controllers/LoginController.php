...
class LoginController extends Controller
{
  ...
  function authenticate()
  {
      if ($_SERVER['REQUEST_METHOD'] === 'POST') {
          ...
          $recaptcha_secret = "6Lc3n1IqAAAAAHkClbuWORNsxl3q9QuvYmli3gjN";
          ...
          if ($returnData) {
              // Check if $returnData is an object
              if (is_object($returnData)) {
                  $hashedPassword = $returnData->password; // Accessing directly since it is an object
  
                  if (password_verify($inputPassword, $hashedPassword)) {
                      // Successful authentication
                      $_SESSION['user_id'] = $returnData->user_id;
                      $_SESSION['username'] = $returnData->username;
                      $_SESSION['full_name'] = $returnData->full_name;
                  ...
                  // Handle the case if selectUser returns an array
                  $_SESSION['login_form_errors_messages'][] = "Unexpected data format.";
                  header('Location: ' . PAGE . 'login');
                  exit();
              }
          } else {
